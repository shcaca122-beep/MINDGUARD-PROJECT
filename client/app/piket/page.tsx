...
export default function DashboardPiket() {
  ...
  const [filterShift, setFilterShift] = useState<'Semua' | 'Shift Pagi' | 'Shift Siang'>('Semua');

  // STATE PROFILE GURU PIKET
  ...
  // DATA DEMO PERIZINAN & UKS
  const [dataTerlambat, setDataTerlambat] = useState([
    { id: '1', nama: 'Risya', kelas: 'XI-1', jenis: 'SAKIT', shift: 'Shift Pagi', periode: '30 Juli 2026 s.d 31 Juli 2026', alasan: 'Demam tinggi', status: 'PENDING' },
    { id: '2', nama: 'Tejar', kelas: 'XI-2', jenis: 'IZIN', shift: 'Shift Siang', periode: '01 Agust 2026 s.d 01 Agust 2026', alasan: 'Acara keluarga', status: 'PENDING' },
  ]);

  const [dataIzinKeluar, setDataIzinKeluar] = useState([
    { id: '1', nama: 'Andi Wijaya', kelas: 'XI TKJ 2', shift: 'Shift Pagi', jamKeluar: '09:30', jamKembali: '10:15', alasan: 'Ambil Buku Paket Ketinggalan', status: 'MENUNGGU' },
  ]);

  const [dataUKS] = useState([
    { id: '1', nama: 'Budi Santoso', kelas: 'X TKJ 1', shift: 'Shift Pagi', keluhan: 'Pusing & Demam', penanganan: 'Istirahat & Minum Obat', status: 'Di UKS' },
  ]);
  ...
  // FILTER SHIFT
  const filteredTerlambat = dataTerlambat.filter(
    item => filterShift === 'Semua' || item.shift === filterShift
  );

  const filteredIzinKeluar = dataIzinKeluar.filter(
    item => filterShift === 'Semua' || item.shift === filterShift
  );

  const filteredUKS = dataUKS.filter(
    item => filterShift === 'Semua' || item.shift === filterShift
  );
      ...
      <main style={{ maxWidth: '1200px', margin: '28px auto', padding: '0 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px', marginBottom: '24px' }}>
          <div style={{ backgroundColor: '#ffffff', padding: '16px 20px', borderRadius: '16px', border: '1px solid #b5d8b6' }}>
            ...
            <div style={{ fontSize: '26px', fontWeight: 'bold', color: '#991b1b', marginTop: '4px' }}>{filteredTerlambat.length} Siswa</div>
          </div>
          <div style={{ backgroundColor: '#ffffff', padding: '16px 20px', borderRadius: '16px', border: '1px solid #b5d8b6' }}>
            <span style={{ fontSize: '11px', color: '#d97706', fontWeight: 'bold' }}>IZIN KELUAR KELAS</span>
            <div style={{ fontSize: '26px', fontWeight: 'bold', color: '#b45309', marginTop: '4px' }}>{filteredIzinKeluar.length} Pengajuan</div>
          </div>
          <div style={{ backgroundColor: '#ffffff', padding: '16px 20px', borderRadius: '16px', border: '1px solid #b5d8b6' }}>
            <span style={{ fontSize: '11px', color: '#2563eb', fontWeight: 'bold' }}>SISWA DI UKS / SAKIT</span>
            <div style={{ fontSize: '26px', fontWeight: 'bold', color: '#1d4ed8', marginTop: '4px' }}>{filteredUKS.length} Orang</div>
          </div>
        </div>
        ...
        <div style={{ backgroundColor: '#ffffff', padding: '24px', borderRadius: '20px', border: '1px solid #b5d8b6', boxShadow: '0 4px 10px rgba(0,0,0,0.04)' }}>
          ...
          {/* FILTER SHIFT */}
          {tabPiket !== 'jurnal' && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '20px', backgroundColor: '#f3f4f6', padding: '10px 16px', borderRadius: '12px', flexWrap: 'wrap' }}>
              <span style={{ fontSize: '12px', fontWeight: 'bold', color: '#374151' }}>⏱️ Filter Shift Piket:</span>
              <div style={{ display: 'flex', gap: '6px' }}>
                {(['Semua', 'Shift Pagi', 'Shift Siang'] as const).map((shift) => (
                  <button
                    key={shift}
                    ...
                    style={{
                      ...
                      fontWeight: 'bold',
                      cursor: 'pointer',
                      backgroundColor: filterShift === shift ? '#1b3b2b' : '#ffffff',
                      color: filterShift === shift ? '#ffffff' : '#374151',
                    ...
                    {filteredTerlambat.length === 0 ? (
                      <tr>
                        <td colSpan={7} style={{ textAlign: 'center', padding: '20px', color: '#6b7280' }}>
                          Tidak ada data perizinan untuk {filterShift}.
                        </td>
                      </tr>
                    ) : (
                      filteredTerlambat.map((item) => (
                        <tr key={item.id} style={{ borderBottom: '1px solid #e2f0e3' }}>
                          ...
                          <td style={{ padding: '12px' }}>
                            <span style={{
                              backgroundColor: item.shift === 'Shift Siang' ? '#fff7ed' : '#eff6ff',
                              color: item.shift === 'Shift Siang' ? '#c2410c' : '#1d4ed8',
                              border: `1px solid ${item.shift === 'Shift Siang' ? '#ffedd5' : '#dbeafe'}`,
                              padding: '3px 8px',
                              borderRadius: '6px',
                              fontWeight: 'bold',
                              fontSize: '11px'
            ...
            <div>
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
                  ...
                  <tbody>
                    {filteredIzinKeluar.length === 0 ? (
                      <tr>
                        <td colSpan={7} style={{ textAlign: 'center', padding: '20px', color: '#6b7280' }}>
                          Tidak ada pengajuan izin keluar untuk {filterShift}.
                        </td>
                      </tr>
                    ) : (
                      filteredIzinKeluar.map((item) => (
                        <tr key={item.id} style={{ borderBottom: '1px solid #e2f0e3' }}>
            ...
            <div>
              ...
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
                  ...
                  <tbody>
                    {filteredUKS.length === 0 ? (
                      <tr>
                        <td colSpan={6} style={{ textAlign: 'center', padding: '20px', color: '#6b7280' }}>
                          Tidak ada catatan UKS untuk {filterShift}.
                        </td>
                      </tr>
                    ) : (
                      filteredUKS.map((item) => (
                        <tr key={item.id} style={{ borderBottom: '1px solid #e2f0e3' }}>
                          <td style={{ padding: '12px', fontWeight: 'bold' }}>{item.nama}</td>
                          <td style={{ padding: '12px' }}>{item.kelas}</td>
                          <td style={{ padding: '12px' }}>
